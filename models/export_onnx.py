...
def convert_upconv_7(model_dir, output_dir):
    for domain in ("photo",):
        in_dir = path.join(model_dir, "upconv_7", domain)
        out_dir = path.join(output_dir, "upconv_7", domain)
        ...
        export_onnx(path.join(in_dir, "noise3_scale2x.pth"), path.join(out_dir, "noise3_scale2x.onnx"))
